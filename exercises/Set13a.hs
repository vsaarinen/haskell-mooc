...
split s
  | length ws >= 2 = Just (head ws, head (tail ws))
  | otherwise = Nothing
  where ws = words s
...
winner scores player1 player2 = do
  ...
  if a >= b then return player1 else return player2
...
balance accountName = BankOp getBalance where
  getBalance (Bank bank) = (Map.findWithDefault 0 accountName bank, (Bank bank))
...
paren c = do
  s <- get
  when (s >= 0) $ modify $ if c == '(' then (+1) else (subtract 1)
...
count k = modify update
  where update [] = [(k,1)]
        update ((xk,num):xs)
          | xk == k = (xk,(num+1)):xs
          | otherwise = (xk,num):update xs
