...
search val tree
  | inTree val tree = Just (buildPath val tree)
  | otherwise = Nothing
  where inTree val Empty = False
        inTree val (Node a l r)
          | a == val = True
          | otherwise = inTree val l || inTree val r
        buildPath val (Node a l r)
          | a == val = []
          | inTree val l = StepL : buildPath val l
          | inTree val r = StepR : buildPath val r
