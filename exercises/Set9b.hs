...
prettyPrint size coords = go coords (replicate (size * size) '.')
  where go [] str = insertNewlines str
        go ((y,x):cs) str = go cs (replaceAt (x+size*(y-1)) 'Q' str)
        replaceAt n x xs = take (n - 1) xs ++ [x] ++ drop n xs
        insertNewlines str = (intercalate "\n" (chunk size str)) ++ "\n"
        chunk _ [] = []
...
danger _ [] = False
danger cand (q:qs) = sameRow cand q || sameCol cand q || sameDiag cand q || sameAntidiag cand q || danger cand qs
...
prettyPrint2 size qs = go (1,1)
  where
    go coord
      | fst coord > size = ""
      | snd coord == size = printPos coord ++ "\n" ++ go (nextRow coord)
      | otherwise = printPos coord ++ go (nextCol coord)
    printPos c
      | elem c qs = "Q"
      | danger c qs = "#"
      | otherwise = "."
