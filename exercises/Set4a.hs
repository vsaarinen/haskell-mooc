...
longest :: Ord a => [[a]] -> [a]
longest = maximumBy (\a b ->
  let lengthComparison = compare (length a) (length b) in
    if lengthComparison == EQ then compare b a else lengthComparison)
...
freqs = foldr (Map.alter increment) Map.empty
  where increment x = case x of Nothing -> Just 1
                                Just n -> Just (n + 1)
