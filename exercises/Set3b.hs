...
sumsFrom n max
  | n == max = [sumFrom 1 n]
  | otherwise = sumFrom 1 n : sumsFrom (n + 1) max

sumFrom i max
  | i == max = i
  | otherwise = i + sumFrom (i + 1) max
...
sorted' _ [] = True
sorted' max (x:xs) = if max > x then False else sorted' x xs
...
sumsOf' sum [] = []
sumsOf' sum (x:xs) = newSum : sumsOf' newSum xs where newSum = sum + x
...
maybeMap _f [] = []
maybeMap f (x:xs) = case f x of Just result -> result : maybeMap f xs
