...
inBoth xs ys = foldr (\x result -> if x `elem` ys then x : result else result) [] xs
...
instance Foldable List where
  foldr f x Empty = x
  foldr f x (LNode y xs) = f y (foldr f x xs)
...
instance Foldable TwoList where
  foldr f x TwoEmpty = x
  foldr f x (TwoNode a b xs) = f a (f b (foldr f x xs))
...
instance Functor Tree where
  fmap f Leaf = Leaf
  fmap f (Node a xs xy) = Node (f a) (fmap f xs) (fmap f xy)

sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node a xs ys) = (sumTree xs) <> a <> (sumTree ys)
